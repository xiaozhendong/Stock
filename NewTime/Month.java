...
package edu.dlnu.liuwenpeng.NewTime;

import java.io.Serializable;
import java.util.Calendar;
import java.util.Date;
import java.util.TimeZone;
import edu.dlnu.liuwenpeng.NewTime.Year;
...
public class Month extends RegularTimePeriod implements Serializable {
	...
	/**
	 * Constructs a new <code>Month</code> instance, based on a date/time and a
	 * time zone. The first and last millisecond values are initially pegged to
	 * the given time zone also.
	 * 
	 * @param time
	 *            the date/time.
	 * @param zone
	...
	public void setMonth(Date time) {
		setMonth(time, RegularTimePeriod.DEFAULT_TIME_ZONE);
	}
	
	public void setMonth(Date time, TimeZone zone) {
		Calendar calendar = Calendar.getInstance(zone);
		calendar.setTime(time);
		this.month = calendar.get(Calendar.MONTH) + 1;
		this.year = calendar.get(Calendar.YEAR);
	...
	public boolean equals(Object obj) {
		...
		if (obj != null) {
			if (obj instanceof Month) {
				...
				return (this.month == target.getMonth() && (this.year == target
						.getYearValue()));
			} else {
				return false;
			}
