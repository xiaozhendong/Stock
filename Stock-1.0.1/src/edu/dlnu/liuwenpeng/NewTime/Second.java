...
public class Second extends RegularTimePeriod implements Serializable {
	...
	public void setSecond(Date time) {
		setSecond(time, RegularTimePeriod.DEFAULT_TIME_ZONE);
	}
	
	public void setSecond(int second, int minute, int hour, int day, int month, int year) {
		setSecond(second, new Minute(minute, hour, day, month, year));
	}
	
	public void setSecond(Date time, final TimeZone zone) {
		Calendar calendar = Calendar.getInstance(zone);
		calendar.setTime(time);
		this.second = (byte) calendar.get(Calendar.SECOND);
		this.minute = (byte) calendar.get(Calendar.MINUTE);
	...
	public void setSecond(int second, Minute minute) {
		if (minute == null) {
			throw new IllegalArgumentException("Null 'minute' argument.");
		}
		this.day = minute.getDay();
