...
import edu.dlnu.liuwenpeng.NewTime.Day;
...
public class Minute extends RegularTimePeriod implements Serializable {
	...
	public void setMinute(int minute, Hour hour) {
		if (hour == null) {
			throw new IllegalArgumentException("Null 'hour' argument.");
		}
		this.minute = (byte) minute;
	...
	public void  setMinute(Date time, TimeZone zone) {
		if (time == null) {
			throw new IllegalArgumentException("Null 'time' argument.");
		}
		if (zone == null) {
	...
	public void setMinute(Date time) {
		// defer argument checking
		setMinute(time, RegularTimePeriod.DEFAULT_TIME_ZONE);
	}
	public void  setMinute(int minute, int hour, int day, int month, int year) {
		setMinute(minute, new Hour(hour, new Day(day, month, year)));
	}
	/**
